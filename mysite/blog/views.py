...
from myAPI.fileAPI import MyFile, upfile_save
...
def upload(request):  
    if request.method == "POST":    # 请求方法为POST时，进行处理  
        upfile = request.FILES.get("myfile", None)    # 获取上传的文件，如果没有文件，则默认为None  
        if not upfile:
            messages.info(request, '没有选择文件！')  
            return HttpResponseRedirect('#')   
        res = upfile_save(filepath, upfile) # 保存上传文件，上传文件名加当前时间
        if 'ok' in res:
            messages.info(request, u'filePATH: %s/%s. add nowTime. Upload Success!' %(filepath, upfile)) 
...
def api_upfile_save(request):
    ...
    if request.method == 'POST':
        ...
        if 'ok' in res:
            res = u'filePATH: %s/%s. add nowTime. Upload Success!' %(filepath, upfile)
    mylist = [{"res" : res }] 
    return JsonResponse(mylist, safe = False) 

# http://localhost:8000/blog/upfile/
def upfile(request):
    """
       使用：'/blog/api_upfile_save/'
       屏蔽：# 'django.middleware.csrf.CsrfViewMiddleware',
    """
    return  render(request, 'blog/upfile.html', context=locals()) 
